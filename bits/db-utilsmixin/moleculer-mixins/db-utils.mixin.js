...
module.exports = {
  ...
  methods: {
    ...
    privilegeChecker(ctx) {
      ...
      if (!privilegesFromToken) {
        // eslint-disable-next-line max-len
        const message = `User does not have any privileges but there are privileges specified for that action! Missing privileges: ${privilegesToCheck.join(', ')}.`
        this.logger.error(message)
      ...
      const matchedPrivileges = privilegesToCheck.filter(
        privilegeToCheck => privilegesFromToken.includes(privilegeToCheck)
      )

...
const { PrivilegesError } = require('../../moleculer-mixins/db-utils.errors')
const { Errors: WebErrors } = require('moleculer-web')

const DbUtilsMixin = require('../../moleculer-mixins/db-utils.mixin')
...
describe('Test "db-utils" mixin', () => {
  const _CONTAINER = {}
  const mockCallByApi = (action, params, privilegesFromToken) => {
    return broker.call(action, params, {
      meta: { calledByApi: true, decodedToken: { privileges: privilegesFromToken } }
    })
  }
  ...
  describe('can check privileges', () => {
    ...
    it('throw when user does not have privileges', () => {
      expect.assertions(2)
      ...
      return mockCallByApi('admin-hello-privileges.hello', {}, undefined)
        .catch((error) => {
          ...
          // eslint-disable-next-line max-len
          expect(error.message).toBe('User does not have any privileges but there are privileges specified for that action! Missing privileges: admin.')
        })
    })
